...
public class MultiLanguageUtil {
    ...
    public static final String SAVE_REFRESH = "save_refresh";
     ...
     * @param languageType
     */
    public void updateLanguage(int languageType, int refreshType) {
        CommSharedUtil.getInstance(mContext).putInt(MultiLanguageUtil.SAVE_LANGUAGE, languageType);
        CommSharedUtil.getInstance(mContext).putInt(MultiLanguageUtil.SAVE_REFRESH, refreshType);
        MultiLanguageUtil.getInstance().setConfiguration();
        EventBus.getDefault().post(new OnChangeLanguageEvent(languageType, refreshType));
    }
    ...
    public String getLanguageName(Context context) {
        ...
        if (languageType == LanguageType.LANGUAGE_EN) {
            ...
            return mContext.getString(R.string.setting_language_simplified_chinese);
        } else if (languageType == LanguageType.LANGUAGE_CHINESE_TRADITIONAL) {
            return mContext.getString(R.string.setting_language_traditional_chinese);
        } else if (languageType == LanguageType.LANGUAGE_KOREAN) {
    ...
    public int getRefreshType() {
        int refreshType = CommSharedUtil.getInstance(mContext).getInt(MultiLanguageUtil.SAVE_REFRESH, RefreshType.REFRESH_ACTIVITY_CLEAR);
        if (refreshType == RefreshType.REFRESH_ACTIVITY_CLEAR) {
            return RefreshType.REFRESH_ACTIVITY_CLEAR;
        } else if (refreshType == RefreshType.REFRESH_ACTIVITY_RECREATE) {
            return RefreshType.REFRESH_ACTIVITY_RECREATE;
        }
        Log.e(TAG, "getLanguageType" + refreshType);
        return refreshType;
    }
